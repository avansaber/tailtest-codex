...
def test_session_start_matcher_covers_every_supported_tailtest_start_source():
...
def test_hook_commands_resolve_scripts_inside_the_plugin_bundle():
    hooks = _hooks()
...
def test_windows_manifest_command_executes_session_start_hook(tmp_path):
    ...
    command_line = f'cmd.exe /D /S /C "{handler["commandWindows"]}"'
...
def test_project_initializer_materializes_runnable_absolute_hook_commands(tmp_path):
    ...
    for groups in installed["hooks"].values():
        for group in groups:
            for handler in group["hooks"]:
                ...
                match = re.search(r'"([^"]+\.py)"', handler["commandWindows"])
                assert match
