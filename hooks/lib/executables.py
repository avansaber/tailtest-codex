...
def resolve_trusted_executable(name: str, project_root: str) -> str | None:
    ...
    process current directory. Tailtest hooks intentionally run for an untrusted
    project, so automatic subprocesses must instead receive an absolute program
    path selected without that ambient current-directory search.
    """
