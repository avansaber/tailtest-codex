...
Treat all pending entries as one cohesive unit. If you wrote a service, a model, and a controller this turn, generate one scenario set for what the system does -- not three disconnected sets.
...
**When the Stop hook context says "existing file -- Do not generate new tests":**
- Run the listed test file
- Report failures only
- Do not write or modify the test file unless you explicitly ask
...
**Files edited outside Codex:** the Stop hook only detects files whose mtime changed during the current Codex turn. Use `/tailtest <file>` to explicitly generate tests for any file edited outside Codex.
